...
mod private {
  ...
  #[cfg(feature = "demia-document")]
  impl Sealed for identity_demia_core::DemiaDocument {}
}
...
mod demia_document {
  ...
  purge_method_for_document_type!(DemiaDocument, purge_method_iota_document);

  #[cfg_attr(not(feature = "send-sync-storage"), async_trait(?Send))]
  #[cfg_attr(feature = "send-sync-storage", async_trait)]
  impl JwkDocumentExt for DemiaDocument {
    async fn generate_method<K, I>(
      &mut self,
      storage: &Storage<K, I>,
      key_type: KeyType,
