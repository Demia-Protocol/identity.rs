...
impl TryFrom<ProvisionalIotaDocument> for IotaDocument {
  ...
  fn try_from(provisional: ProvisionalIotaDocument) -> std::result::Result<Self, Self::Error> {
    ...
    for controller_id in document
      ...
      .flatten()
    {
      DemiaDID::check_validity(controller_id).map_err(|_| {
        Error::SerializationError(
...
impl IotaDocument {
  ...
  /// Constructs an empty DID Document with a [`DemiaDID::placeholder`] identifier
  ...
  pub fn new(country_code: &CountryCode, network: &NetworkName) -> Self {
    Self::new_with_id(DemiaDID::placeholder(country_code, network))
  }

  /// Constructs an empty DID Document with the given identifier.
  pub fn new_with_id(id: DemiaDID) -> Self {
  ...
  pub fn controller(&self) -> impl Iterator<Item = &DemiaDID> + '_ {
    ...
    core_did_controller_iter.map(DemiaDID::from_inner_ref_unchecked)
  }
...
use crate::block::address::Address;
  ...
  impl IotaDocument {
    ...
    fn set_controller_and_governor_addresses(&mut self, alias_output: &AliasOutput, country_code: &CountryCode, network_name: &NetworkName) {
      ...
      let controller_did: Option<DemiaDID> = match alias_output.state_controller_address() {
        Address::Alias(alias_address) => Some(DemiaDID::new(alias_address.alias_id(), country_code, network_name)),
        _ => None,
      };
...
mod tests {
  ...
  fn valid_did() -> DemiaDID {
    "did:demia:0xAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
      .parse()
      .unwrap()
  }

  fn generate_method(controller: &DemiaDID, fragment: &str) -> VerificationMethod {
  ...
  fn test_unpack_empty() {
    ...
    let controller_did: DemiaDID = valid_did();

    // VALID: unpack empty, deactivated document.
    let did: DemiaDID = "did:demia:0xBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB"
      .parse()
      .unwrap();
    ...
    let json: String = format!(
      r#"{{"doc":{{"id":"{did}","controller":"{controller_did}"}},"meta":{{"deactivated":true,"governorAddress":"dmia1pz424242424242424242424242424242424242424242424242425uk7x2y","stateControllerAddress":"dmia1pz424242424242424242424242424242424242424242424242425uk7x2y"}}}}"#
    );
  ...
  fn deserializing_id_from_other_method_fails() {
    const JSON_DOC_INVALID_ID: &str = r#"
    {
      "doc": {
        ...
        "verificationMethod": [
          {
            "id": "did:demia:0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa#issuerKey",
            "controller": "did:demia:0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
    ...
    {
      "doc": {
        "id": "did:demia:0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "verificationMethod": [
          {
            "id": "did:demia:0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa#issuerKey",
            "controller": "did:demia:0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  ...
  fn deserializing_controller_from_other_method_fails() {
    ...
    {
    "doc": {
      "id": "did:demia:usa:rms:0x7591a0bc872e3a4ab66228d65773961a7a95d2299ec8464331c80fcd86b35f38",
      "controller": "did:example:usa:rms:0xfbaaa919b51112d51a8f18b1500d98f0b2e91d793bc5b27fd5ab04cb1b806343",
      "verificationMethod": [
        {
          "id": "did:demia:usa:rms:0x7591a0bc872e3a4ab66228d65773961a7a95d2299ec8464331c80fcd86b35f38#key-2",
          "controller": "did:demia:usa:rms:0x7591a0bc872e3a4ab66228d65773961a7a95d2299ec8464331c80fcd86b35f38",
          "type": "Ed25519VerificationKey2018",
  ...
  "doc": {
    "id": "did:demia:usa:rms:0x7591a0bc872e3a4ab66228d65773961a7a95d2299ec8464331c80fcd86b35f38",
    "controller": "did:demia:usa:rms:0xfbaaa919b51112d51a8f18b1500d98f0b2e91d793bc5b27fd5ab04cb1b806343",
    "verificationMethod": [
      {
        "id": "did:demia:usa:rms:0x7591a0bc872e3a4ab66228d65773961a7a95d2299ec8464331c80fcd86b35f38#key-2",
        "controller": "did:demia:usa:rms:0x7591a0bc872e3a4ab66228d65773961a7a95d2299ec8464331c80fcd86b35f38",
  ...
  fn try_from_doc_metadata() {
    ...
    const DOC_JSON_NOT_IOTA_DOCUMENT_BECAUSE_OF_CONTROLLER: &str = r#"
    {
      ...
      "verificationMethod": [
        {
          "id": "did:demia:usa:rms:0x7591a0bc872e3a4ab66228d65773961a7a95d2299ec8464331c80fcd86b35f38#key-2",
          "controller": "did:demia:rms:0x7591a0bc872e3a4ab66228d65773961a7a95d2299ec8464331c80fcd86b35f38",
          "type": "Ed25519VerificationKey2018",
          "publicKeyMultibase": "z7eTUXFdLCFg1LFVFhG8qUAM2aSjfTuPLB2x9XGXgQh6G"
        }
      ]
